...
export const resetPasswordRequest = createAsyncThunk('resetPassword/request',
   async ({ email }) => {
      try {
         const response = await axios.post("http://localhost:3000/auth/reset-password-request", { email })         
...
export const resetPassword = createAsyncThunk('resetPassword',
   async ({ email, newPassword, confirmPassword, resetToken }, { rejectWithValue }) => {
      try {
         const response = await axios.post("http://localhost:3000/auth/reset-password", { email, newPassword, confirmPassword, resetToken }, {withCredentials: true})
         return response.data;
      } catch (error) {
         console.error("Failed to reset password");
